...
<!-- Панель поиска и управления деревом (без кнопки добавления) -->
...
<!-- Скрипты для дерева (AJAX-загрузка устройств и временных зон) -->
<script>
$(document).ready(function() {
    ...
    $(document).on('click', '.tree-node-category', function(e) {
        ...
        loadDevices($li, $li.data('category-id'));
    });
    
    $(document).on('click', '.tree-node-device', function(e) {
        if ($(e.target).closest('.tree-actions, .action-btn, a').length) return;
        var $li = $(this).closest('.tree-device');
        loadTimezones($li, $li.data('category-id'), $li.data('device-id'));
    });
